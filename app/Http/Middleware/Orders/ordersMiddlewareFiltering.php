...
/** --------------------------------------------------------------------------------
 * This middleware class handles filter processing for orders
 *
 * @package    Grow CRM
 * @author     NextLoop
 *----------------------------------------------------------------------------------*/
...
class ordersMiddlewareFiltering {
    ...
    public function handle($request, Closure $next) {
        ...
        //filter handling
        if (request()->routeIs('orders.search')) {
    ...
    private function saveFilter() {
        ...
        if (!$filter = \App\Models\Filter::where('filter_userid', auth()->id())
            ->where('filter_type', 'orders')->first()) {
