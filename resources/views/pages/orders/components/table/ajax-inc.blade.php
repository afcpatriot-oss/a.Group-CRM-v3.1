<tr id="order_{{ $order->id }}" class="{{ $order->pinned_status ?? '' }}">
    ...
    <td class="orders_col_checkbox checkitem" id="orders_col_checkbox_{{ $order->id }}">
        <span class="list-checkboxes display-inline-block w-px-20">
            <input type="checkbox" id="listcheckbox-orders-{{ $order->id }}" name="ids[{{ $order->id }}]"
    ...
    <td class="orders_col_id"><a href="/orders/{{ $order->id }}">{{ $order->order_number }}</a></td>
    @endif

    <td class="orders_col_subject">
        <a href="/orders/{{ $order->id }}">{{ str_limit($order->order_number ?? '---', 35) }}</a>
    </td>
    ...
    <td class="orders_col_user">
        ...
        <a href="javascript:void(0);"
            ...
            data-toggle="modal" data-target="#commonModal" data-url="{{ url('contacts/'.$order->id) }}"
            ...
            data-action-ajax-loading-target="commonModalBody">{{ $order->first_name ?? '---' }} {{ $order->last_name ?? '' }}
        </a>
        @else
    ...
    <td class="orders_col_client">{{ str_limit($order->client_name ?? '---', 15) }}</td>
    @endif

    <td class="orders_col_date">{{ runtimeDate($order->order_created ?? $order->created_at) }}</td>

    <td class="orders_col_status">
        <span class="label label-outline-default">{{ runtimeLang('order_status') }}</span>
    </td>
    ...
    <td class="orders_col_action actions_column">
        <span class="list-table-action font-size-inherit">
            ...
            <button type="button" title="{{ cleanLang(__('lang.edit')) }}"
                ...
                data-loading-target="commonModalBody" data-modal-title="{{ cleanLang(__('lang.edit_order')) }}"
                data-action-url="{{ urlResource('/orders/'.$order->id) }}" data-action-method="PUT"
                data-action-ajax-class="js-ajax-ux-request" data-action-ajax-loading-target="orders-td-container">
                <i class="sl-icon-note"></i>
            </button>
            @endif

            <a href="/orders/{{ $order->id }}" title="{{ cleanLang(__('lang.view')) }}"
